  ...
  return got(url, {
    ...
    timeout:
      typeof requestTimeout === 'number'
        ...
        : requestTimeout,
    retry: typeof retry === 'number' ? { limit: retry } : retry,
  });
}
...
async function one() {
  ...
  const { body } = await httpClient.get(url, { responseType: 'json' });
  ...
  return new Promise((resolve) => {
    ...
    if (TG_BOT_TOKEN && TG_USER_ID) {
      ...
      if (TG_PROXY_HOST && TG_PROXY_PORT) {
        // 仅在实际配置 Telegram 代理时加载代理依赖。
        const proxyAgentModule = require('https-proxy-agent');
        const HttpsProxyAgent =
          proxyAgentModule.HttpsProxyAgent || proxyAgentModule;
        const proxyUrl =
          `http://${TG_PROXY_AUTH || ''}${TG_PROXY_HOST}:${TG_PROXY_PORT}`;
        options.agent = {
...
function webhookNotify(text, desp) {
  return new Promise((resolve) => {
    ...
    httpClient.request(formatUrl, options).then(
      (resp) => {
